...
interface ImportCSVDataUseCase{
    suspend operator fun invoke(launcher: ImportDataUseCase.ResultLauncherWrapper){

    }
}
class NoOpImportCSVDataUseCase: ImportCSVDataUseCase
class ImportCSVDataUseCaseImpl(
    private val context: Context,
    private val toastMaker: ToastMaker,
    private val knownExerciseService: KnownExerciseService,
    private val workoutService: WorkoutService,
): ImportCSVDataUseCase {
    override suspend operator fun invoke(launcher: ImportDataUseCase.ResultLauncherWrapper) {
        work(launcher)
    }

        private suspend fun work(launcher: ImportDataUseCase.ResultLauncherWrapper): Any {
        ...
        fun readFile(uri: Uri?) {
            ...
            try {
                ...
                val text: String = Scanner(inputStream).useDelimiter("\\A").next()
                // read from `String`

                val rows: List<List<String>> = csvReader().readAll(text)


                // Here is where the magic happens
            val sets = rows.subList(1,rows.size-1).map {
                Log.d("Spreadsheet", "${it.joinToString("-----")}")

                WorkoutSet(it[0],it[1],it[2],it[4].toDouble(),it[3].toDouble())
            } // Read File and Construct Local Objects
...
//            System.out.println(row[5]);
        ...
        fun SetsToEverything( sets: List<WorkoutSet>): MutableList<WorkoutDay> {
